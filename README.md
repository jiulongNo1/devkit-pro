...
| **正则表达式测试** | 正则匹配测试、实时高亮、多标志位支持、Qt正则模式、语法差异提示、C++代码生成 |
...
3. 实时查看匹配结果和高亮显示
4. 支持复制所有匹配结果
5. **Qt 正则模式**（切换到 Qt 模式后）：
   - 自动检测 Qt 正则语法兼容性问题
   - 显示 Qt 兼容的正则表达式格式
   - 一键生成 Qt C++ 代码（QRegularExpression）
   - 展示 Qt 正则与 PCRE/JS 语法差异表
   - 展示 Qt PatternOptions 映射表
...
1. 选择示例模板或自定义字段
2. 添加字段：字段名、类型（char/short/int/long/long long/float/double/bool/指针/自定义）、数组长度
3. 计算并显示：
   - 单个对象大小（sizeof）
   - 100万和1亿对象的内存占用（MB/GB）
   - 缓存行影响分析（64字节缓存行能容纳多少个对象）
   - Padding 占比和紧凑度评分
4. 提供优化建议：
   - 建议字段重排顺序（大字段优先）以减少 padding
   - 标注哪些字段导致了 padding
   - 展示优化前后的内存对比
5. 布局可视化：色块展示当前布局和优化后布局对比
6. 自动生成优化后的结构体定义代码

### i18n翻译辅助

1. 键命名生成 Tab：
   - 输入中文文本，自动生成多种风格的翻译键名
   - 支持：下划线风格、驼峰风格、点分隔风格、命名空间风格
   - 点击生成的键名可添加到翻译列表
2. 格式生成 Tab：
   - 键值对表格：可编辑键名、中文、英文
   - 支持 6 种输出格式：Qt .ts、JSON、JSON嵌套、YAML、.properties、gettext .po
   - 一键复制或下载文件
3. 批量导入 Tab：
   - 粘贴每行一条文本
   - 自动为每条生成翻译键名建议
...
1. 输入两个版本号进行比较
2. 实时显示比较结果：大号比较符号 + 文字描述
3. 逐段对比表格：高亮差异位
4. 支持多种格式：
   - 语义化版本：1.2.3 / 1.2.3-alpha / 1.2.3-beta.2
   - 带前缀：v1.2.3 / Ver1.2.3
   - 四段式：1.2.3.4
5. 批量排序：输入多行版本号，支持升序/降序
6. 生成 C++ 比较代码：自定义函数 / std::tuple / C++20 views
